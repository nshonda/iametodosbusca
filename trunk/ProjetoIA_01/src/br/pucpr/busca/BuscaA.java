...
public class BuscaA {
	 ...
	 * @throws NóNãoEncontradoExceção
	 */
	public Node buscarA(Node inicio, Node fim)
			throws NóNãoEncontradoExceção {
		logger.debug("Começar o método de busca A*.");
		...
		nó.calculaTempoRestante(fim);
		nó.calculaTempoTotal();
		
		adicionaNodeNaFila(ligacoes, nó, fim);
	 ...
	 * @throws NóNãoEncontradoExceção
	 */
	private void adicionaNodeNaFila(List<Edge> ligacoes, Node pai, Node destino)
			throws NóNãoEncontradoExceção {
		...
		for (Edge edge : ligacoes) {
			...
			if (!nó.isVisitado()) {
				if (nó.getPai() == null){
					nó.setPai(pai);
					nó.calculaTempoTotal();
					nó.calculaTempoRestante(destino);
				}
		...
		logger.debug("Imprimindo Lista de Adjacencias");
		for (Node No : adjacentes) {
